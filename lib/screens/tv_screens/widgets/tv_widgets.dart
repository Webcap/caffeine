...
import 'package:reelriot/models/videos.dart';
...
// ─── Design tokens for TV streaming section (design.json) ────────────────────
class _TVStreamingDesign {
...
class TVShowsFromWatchProvidersState extends State<TVShowsFromWatchProviders> {
  ...
  Widget build(BuildContext context) {
    ...
    final textPrim = isDark
        ? _TVStreamingDesign.textPrimDark
        : _TVStreamingDesign.textPrimLight;
      ...
      children: [
        Padding(
          padding: const EdgeInsets.only(
            left: _TVStreamingDesign.screenPadH,
            right: _TVStreamingDesign.screenPadH,
            top: _TVStreamingDesign.space2,
            bottom: _TVStreamingDesign.space3,
          ),
          child: Row(
            children: [
              const LeadingDot(),
        ...
        SizedBox(
          ...
          child: ListView(
            ...
            padding: const EdgeInsets.symmetric(
                horizontal: _TVStreamingDesign.screenPadH),
            children: [
...
// ─── Design tokens for StreamingServicesTVShows screen ───────────────────────
class _TVStreamingScreenDesign {
...
class StreamingServicesTVShows extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isDark = themeMode == 'dark' || themeMode == 'amoled';
    final bg = isDark
        ? _TVStreamingScreenDesign.bgCanvasDark
        : _TVStreamingScreenDesign.bgCanvasLight;
    final surface = isDark
        ? _TVStreamingScreenDesign.bgSurfaceDark
        : _TVStreamingScreenDesign.bgSurfaceLight;
    final textPrim = isDark
        ? _TVStreamingScreenDesign.textPrimDark
        : _TVStreamingScreenDesign.textPrimLight;
    final border = isDark
        ? _TVStreamingScreenDesign.borderDark
        : _TVStreamingScreenDesign.borderLight;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: Padding(
          padding: const EdgeInsets.only(left: _TVStreamingScreenDesign.space2),
          child: Material(
            color: isDark ? _TVStreamingScreenDesign.iconBgDark : border,
      ...
      body: ParticularStreamingServiceTVShows(
        ...
        api: Endpoints.watchProvidersTVShows(providerId, 1, lang, region),
      ),
    );
  }
}
...
class TVStreamingServicesWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isDark = themeMode == 'dark' || themeMode == 'amoled';
    final surface = isDark
        ? _TVStreamingDesign.bgSurfaceDark
        : _TVStreamingDesign.bgSurfaceLight;
    final border =
        isDark ? _TVStreamingDesign.borderDark : _TVStreamingDesign.borderLight;
    final textPrim = isDark
        ? _TVStreamingDesign.textPrimDark
        : _TVStreamingDesign.textPrimLight;

    return Padding(
      padding: const EdgeInsets.only(right: _TVStreamingDesign.space3),
      child: Material(
        color: surface,
        borderRadius: BorderRadius.circular(_TVStreamingDesign.radiusMd),
        clipBehavior: Clip.antiAlias,
        child: InkWell(
          onTap: () {
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => StreamingServicesTVShows(
                  providerId: providerID,
                  providerName: title,
                ),
              ),
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: const [_TVStreamingDesign.shadowCard],
            ),
            padding: const EdgeInsets.symmetric(
              horizontal: _TVStreamingDesign.space3,
              vertical: _TVStreamingDesign.space2,
            ),
            child: Row(
              ...
              children: [
                ...
                const SizedBox(width: _TVStreamingDesign.space2),
                Expanded(
                  child: Text(
                    title,
                    style: TextStyle(
...
// ─── Empty state card for streaming TV (design.json) ─────────────────────────
class _EmptyStreamingTVCard extends StatelessWidget {
  final String message;

  const _EmptyStreamingTVCard({required this.message});
